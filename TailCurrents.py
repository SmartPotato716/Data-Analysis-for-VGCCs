def tailcurrents(filepath, current_start_time, current_end_time):
    '''
    Perform Batch analysis to the abf files, parsing out the absolute peak currents within tail currents and plot the opening propabilities.
    ...
    for file in file_list:
        ...
        for sweepnumber in abf.sweepList:  # loop through the sweeps
            ...
            time = abf.sweepX
            # Find indices corresponding to the time range for the voltage steps, here we are setting it to be between 0.06s to 0.109s.
            voltage_start_index = np.where(time >= 0.06)[0][0]
            voltage_end_index = np.where(time <= 0.09)[0][-1]
            ...
            current_start_index = np.where(time >= current_start_time)[0][0]
            current_end_index = np.where(time <= current_end_time)[0][-1]            
    ...
    output.to_csv(f"{BatchExportPath}/BatchTailCurrents_{Batchlabel}.csv", index = False)
    plt.savefig(f"{BatchExportPath}/BatchTailCurrents_{Batchlabel}.png", dpi=300)
